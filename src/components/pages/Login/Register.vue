<template>
  <div class="register">
    <el-form :model="regForm" :rules="rules" ref="regForm">
      ...
      <el-form-item label="身份" prop="userole" label-width="80px">
        <el-select v-model="regForm.userole" clearable placeholder="请选择">
...
<script>
  ...
  import { register, queryName, } from '@/api/login.js'
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        ...
        let password = CryptoJS.MD5(this.regForm.password).toString()
        let addtime = moment().format('YYYY-MM-DD HH:mm:ss')
        let edittime = addtime
        let userole = this.regForm.userole
